...
X68000 Z のエミュレータには、ZUSB 機能を提供するためにオリジナルの X68000 では未使用だったアドレス領域 ($EC0000 ～ $EC7FFF) に ZUSB 制御レジスタと USB バッファメモリが配置されています。
このドキュメントはこのレジスタ仕様について説明します。
...
#### エンドポイントパイプ
  ZUSB が USB デバイスが持つエンドポイントに対してデータの送受信を行うインターフェースをエンドポイントパイプと呼びます。  
  ZUSB は各チャネルごとに 0～7 の 8 個のパイプを持ちます。  
  USB デバイスは IN/OUT それぞれに 15 個ずつとコントロールパイプ用の 1 個、合わせて最大 31 個のエンドポイントを持つことができますが、ZUSB ではその中から 8 個を選択してパイプに割り当て、そのパイプが X68000 のメモリとの間でデータのやり取りを行います。  
