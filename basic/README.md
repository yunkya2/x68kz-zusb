...
* 機能
  * パイプからデータを読み込みます。
    ...
    * len -- 読み込むデータ長
    * epno -- パイプ番号 (0～7)
    * pos -- データ読み込み時に使用する USB バッファ領域内のオフセット
      * &h000～&HF7F の値を指定します。省略すると 0 になります。
      * 複数のパイプに同時にデータを読み書きする場合は、それぞれのパイプが使用する USB バッファが重ならないようにオフセットを指定してください。
  * 戻り値として以下の値を返します。
...
* 機能
  * パイプにデータを書き込みます。
    ...
    * len -- 書き込むデータ長
    * epno -- パイプ番号 (0～7)
    * pos -- データ書き込み時に使用する USB バッファ領域内のオフセット
      * &h000～&HF7F の値を指定します。省略すると 0 になります。
      * 複数のパイプに同時にデータを読み書きする場合は、それぞれのパイプが使用する USB バッファが重ならないようにオフセットを指定してください。
  * 戻り値として以下の値を返します。
